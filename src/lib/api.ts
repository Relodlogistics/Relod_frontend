...
export const api = {
  ...
  // uses whatever GEOCODING_PROVIDER is active server-side (see
  // GeocodingService.resolveAddress). Returns null if it can't be resolved.
  ...
  registerPushToken: (token: string, fcmToken: string) =>
    request<{ registered: true }>('/notifications/push-token', {
      method: 'POST',
      token,
...
export interface MatchingCarrier {
  ...
  // The matched truck's position, shown pre-booking on find-carriers/page.tsx
  // ("Track truck") — being matched is treated as enough of a relationship
  // to see roughly where the truck is.
  matchLat: number;
  matchLng: number;
  score: number;
  reasons: string[];
